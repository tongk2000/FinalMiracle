...
// ===== #129. FileManager 클래스 생성하기 =====  // File upload를 사용하기 위해서 Spring 설정을 pom.xml(11번)에서 해야한다.
@Repository
public class FileManager {
	...
	// 파일 다운로드
	// saveFilename : 서버에 저장된 파일명(DB상 숫자 파일명)
	// originalFilename : 클라이언트가 업로드한 파일명(DB상 문자 오리지널 파일명)
	// path : 서버에 저장된 경로(.metadata)
	public boolean doFileDownload(String saveFilename, String originalFilename, String path, HttpServletResponse response) {
        ...
        try {
        	...
        	originalFilename = new String(originalFilename.getBytes("euc-kr"),"8859_1");  // 파일명이 한글일 경우 파일명이 깨지지 않게 하기 위한 것.
