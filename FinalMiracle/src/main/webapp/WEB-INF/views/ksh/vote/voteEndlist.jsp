...
<form class="form-inline" id="listFrm" name="listFrm" action="<%= request.getContextPath() %>/voteEndList.mr" method="get" enctype="multipart/form-data" style="font-family: verdana;">
					...
					<div style="width: 95%; margin-top: 2%;">
						<div style="width: 20%; border: 1px solid lightgray; padding: 10px;">
							...
							<c:forEach var="voteitemvo" items="${voteItemList}" varStatus="status">
								...
								<c:if test="${voteidx eq voteitemidx}">
									<ul style="margin:10px; padding:0px;">
										<li style="margin:10px; padding:0px;">${voteitemvo.item}&nbsp;:&nbsp;${voteitemvo.votenum}표
									</ul>
									<c:set var="votesum" value="${votesum + voteitemvo.votenum}" />
								</c:if>
							</c:forEach>
							...
							<div style="text-align: center;">
								...
								<button class="btn btn-xs btn-info" type="button" data-toggle="modal" data-target="#chartModal" onclick="callChart('${votevo.IDX}')"><span class="glyphicon glyphicon-stats"></span>&nbsp;차트보기</button>
							</div>
						</div>
						<br/>
						${votevo.CONTENT}
