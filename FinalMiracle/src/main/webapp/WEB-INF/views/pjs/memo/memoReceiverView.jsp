					...
					<c:set var="receiver" value="${map}"></c:set>
						<tr> <!-- IDX, SUBJECT, CONTENT, SENDER, SSTATUS, name, img, w.status -->
							<c:if test="${receiver.status == 2}">
								<th style="background-color:#337ab7;">유저 :  </th><td><img src="<%= request.getContextPath() %>/resources/images/${receiver.img}" class="imgs"> ${receiver.name} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[${teamNum}팀  팀장]</td> 
							</c:if>
							<c:if test="${receiver.status == 1}">
								<th style="background-color:#337ab7;">유저 :  </th><td><img src="<%= request.getContextPath() %>/resources/images/${receiver.img}" class="imgs"> ${receiver.name} &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;[${teamNum}팀   팀원]</td> 
							</c:if>
						</tr>
						<tr>
							<th style="background-color:#337ab7;">제목 :  </th><td><input type="text" name="subject" id="subject" readonly value="${receiver.subject}"/></td>
						</tr>
						<tr>
							<th style="background-color:#337ab7;">내용 :  </th><td><div style="width:700px; height:500px;">${receiver.content}</div></td>
						</tr>
					</tbody>
				</table>
			</div>
